...
function KanbanColumn({
  ...
  async function handleDeleteColumn() {
    ...
    const confirmed = confirm(
      `"${column.title}" sütununu silmek istediğine emin misin?`
    );

    if (!confirmed) return;

    const res = await fetch(`/api/projects/${projectId}/columns/${column.id}`, {
      method: "DELETE",
    });
    ...
    if (!res.ok) {
      setError(data.message || "Sütun silinemedi");
      return;
    }
  ...
  return (
    <div
      ref={setNodeRef}
      className={`w-80 shrink-0 rounded-3xl border p-4 transition ${isOver
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        {isOwner && (
          <button
            ...
            className="rounded-xl border border-red-500/20 px-3 py-1 text-xs font-medium text-red-300 transition hover:bg-red-500/10"
          >
            Sil
          </button>
        )}
...
function SortableCard({
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className={`cursor-grab rounded-2xl border border-white/10 bg-black p-4 shadow-sm transition active:cursor-grabbing ${isDragging || isActive
